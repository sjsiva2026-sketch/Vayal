...
// After submit → Alert → navigate to OwnerHome (Requests tab)
...
export default function WorkComplete({ navigation, route }) {
  ...
  const handleComplete = async () => {
    ...
    if (hc > booking.hectareRequested * 3) {
      Alert.alert(
        'Seems High',
        `Requested was ${booking.hectareRequested} ha. Confirm?`,
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Yes', onPress: () => doComplete(hc) },
        ],
      );
      return;
    }
    ...
    try {
      ...
      if (result.alreadyCompleted) {
        Alert.alert('Already Done', 'This job was already marked complete.', [
          { text: 'Go to Requests', onPress: goToRequests },
        ]);
        return;
      }
      ...
      // ── After submit → navigate to Requests screen ───────────────────
      Alert.alert(
        '✅ Work Completed!',
        `Commission: Rs.${result.commissionAmount}\n` +
        `Pay within 24 hours to avoid lock.\n` +
        `Deadline: ${new Date(result.paymentDeadline).toLocaleString('en-IN')}`,
        [
          {
            text: 'Go to Requests',
            onPress: goToRequests,   // ← goes to OwnerHome → Requests tab
          },
        ],
        { cancelable: false },
      );
        ...
        >
          <LinearGradient colors={['#145A3E', '#1C7C54']} style={s.header}>
            ...
            <Text style={s.headerSub}>
              Enter actual hectares — 24h commission timer starts NOW
            </Text>
          </LinearGradient>
          ...
          <View style={s.timerWarn}>
            <Text style={s.timerWarnTxt}>
              ...
              Pay before it expires to avoid account lock.
            </Text>
          </View>
          ...
          {parseFloat(hectareDone) > 0 && (
            <View style={s.section}>
              <View style={s.commCard}>
                ...
                <Text style={s.commRate}>
                  Rs.{CONFIG.COMMISSION_PER_HECTARE}/ha × {hectareDone} ha
                </Text>
          ...
          <View style={s.section}>
            ...
            >
              <Text style={s.submitBtnTxt}>
                {loading   ? 'Processing...'
                : completed ? '✅ Submitted'
                :             'Submit & Start 24h Timer'}
              </Text>
            </TouchableOpacity>

            {/* Manual navigate button if alert dismissed */}
            {completed && (
              <TouchableOpacity
                style={s.requestsBtn}
                ...
                activeOpacity={0.88}
              >
                <Text style={s.requestsBtnTxt}>← Back to Requests</Text>
              </TouchableOpacity>
            )}
          </View>
...
const s = StyleSheet.create({
  ...
  requestsBtn:   { backgroundColor: '#F0FDF4', borderRadius: rs(14), paddingVertical: rs(14), alignItems: 'center', borderWidth: rs(1.5), borderColor: '#22C55E' },
  requestsBtnTxt:{ color: '#065F46', fontSize: rf(15), fontWeight: '700' },
});
