...
// 'Pay Commission' screen name = 'PayCommission' (works in both locked & unlocked)
const ACTIONS = [
  ...
  { icon: '💰', label: 'Pay Commission', sub: 'UPI Pay',       screen: 'PayCommission',    from: '#FFEDD5', to: '#FED7AA' },
  { icon: '👤', label: 'My Profile',     sub: 'Settings',      screen: 'OwnerProfile',     from: '#E0E7FF', to: '#C7D2FE' },
];

// Screens accessible even when account is locked
const FREE_SCREENS = ['PayCommission', 'OwnerProfile'];
  ...
  return (
    <SafeAreaView style={s.safe}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
          ...
          <View style={s.headerRow}>
            <View style={s.headerLeft}>
              ...
              <View style={s.locPill}>
                <Text style={s.locTxt}>
                  📍 {userProfile?.taluk || '—'}  ·  +91 {userProfile?.phone || '—'}
                </Text>
              </View>
            </View>
        ...
        <View style={s.grid}>
          {ACTIONS.map((a, i) => {
            const locked = isLocked && !FREE_SCREENS.includes(a.screen);
            const last3  = (i + 1) % 3 === 0;
            return (
              <TouchableOpacity
                key={a.screen}
                ...
                onPress={() => {
                  if (locked) {
                    Alert.alert('🔒 Locked', 'Pay commission to unlock all features.', [
              ...
              >
                <LinearGradient
                  colors={locked ? ['#F3F4F6', '#E5E7EB'] : [a.from, a.to]}
                  style={s.actionGrad}
                >
...
const s = StyleSheet.create({
  ...
  lockIconBox:  { width: 44, height: 44, borderRadius: 12, backgroundColor: '#FEE2E2', alignItems: 'center', justifyContent: 'center' },
