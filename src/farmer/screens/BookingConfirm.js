// src/farmer/screens/BookingConfirm.js
// FIXED: "View My Bookings" button navigates correctly
// UPI: receiver name from CONFIG
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={s.section}>
          ...
          {machine?.ownerPhone ? (
            <PhoneConnect
              phone={machine.ownerPhone}
              name={machine.ownerName || 'Owner'}
              role="Machine Owner 🚜"
            />
          ) : (
        ...
        <View style={s.btnGroup}>
          {/* FIXED: Navigate to MyBookings (correct screen name) */}
          <TouchableOpacity
            style={s.btnPrimary}
...
const s = StyleSheet.create({
  ...
  otpCard:       { backgroundColor: '#0F4C2A', borderRadius: rs(16), padding: rs(24), alignItems: 'center' },
  ...
  otpWarn:       { marginTop: rs(16), backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: rs(8), paddingHorizontal: rs(14), paddingVertical: rs(8) },
  otpWarnTxt:    { fontSize: rf(12), color: 'rgba(255,255,255,0.7)', textAlign: 'center' },
  waitBox:       { backgroundColor: '#FFF9E6', borderRadius: rs(12), padding: rs(14), borderLeftWidth: rs(4), borderLeftColor: '#F59E0B' },
  waitTxt:       { fontSize: rf(13), color: '#92400E' },
  btnGroup:      { paddingHorizontal: H_PAD, marginTop: rs(24) },
  btnPrimary:    { backgroundColor: COLORS.primary, borderRadius: rs(14), paddingVertical: rs(15), alignItems: 'center', marginBottom: rs(10) },
  btnPrimaryTxt: { color: '#fff', fontSize: rf(15), fontWeight: '800' },
  btnOutline:    { borderRadius: rs(14), paddingVertical: rs(15), alignItems: 'center', borderWidth: rs(1.5), borderColor: COLORS.border, backgroundColor: '#fff' },
